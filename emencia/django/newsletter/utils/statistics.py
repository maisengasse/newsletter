...
def get_newsletter_statistics(newsletter):
    ...
    # recipients = post_sending_status.filter(status=Status.SENT, newsletter=newsletter).count()
    recipients = newsletter.mailing_list.expedition_set().count()
    mails_sent = min(post_sending_status.filter(status=Status.SENT).count(), recipients)
